# 03 â€“ Multi-Tier App (ALB + ASG + RDS)
...
Each tier sits in its own layer of the VPC, with security groups enforcing that traffic only flows one tier to the next â€” never skipping a layer or reaching the database directly from the internet.

# Why This Pattern
1.  Scalability â€” the ASG adds/removes EC2 instances automatically based on load, instead of running a fixed, potentially oversized (or undersized) fleet.
2.  High availability â€” resources are spread across multiple Availability Zones (AZs), so a single AZ failure doesn't take the app down.
3.  Security by layering â€” only the ALB is internet-facing; app servers and database are never directly reachable from the public internet.
4.  Decoupled tiers â€” web/app tier and database tier can be scaled,patched, and secured independently.
...
1. VPC with 3 subnet tiers: Public subnets (ALB, NAT Gateway), private app subnets (EC2/ASG), private DB subnets (RDS) â€” each tier duplicated across â‰¥2 AZs.
...
6. Auto Scaling Group (ASG): Manages the fleet of EC2 app instances â€” launches/terminates based on a scaling policy, spread across AZs.
7. Launch Template: Defines what each ASG instance looks like â€” AMI, instance type, user data (bootstrap script), IAM instance profile, security group.
8. RDS (Multi-AZ): The database tier â€” a primary instance with a synchronously replicated standby in a second AZ for automatic failover.
...
1. Design and build the VPC CIDR block, 3 subnet tiers Ã— 2+ AZs (public, app/private, DB/private).
Internet Gateway attached; NAT Gateway(s) in the public subnet(s) â€”one per AZ for HA, or one shared to save cost in non-prod.
...
AMI (custom or base + user data bootstrap), instance type, key pair,app-sg, IAM instance profile (least-privilege â€” e.g., permission to read the DB secret from Secrets Manager, nothing else).
...
7. Test end-to-end
Hit the ALB DNS name / custom domain â€” confirm the app responds and can read/write to RDS.
...
# Failure testing:
1. Terminate an app instance manually â€” confirm ASG replaces it and ALB stops routing to it during the outage (health checks failing).
2. Trigger a scaling event (e.g., stress CPU) and confirm the ASG scales out, then back in when load drops.

# Lessons Learned
1. Security group chaining, not CIDR-based rules â€” referencing alb-sg / app-sg as the source in the next tier's inbound rule (not a CIDR range) means it stays correct even as instances scale up/down or get new IPs.
2. RDS must NOT be in public subnets, and "Publicly accessible" should be set to No â€” this is a common oversight that exposes the database directly to the internet.
3. Don't bake DB credentials into the AMI or user data in plaintext â€”pull them from Secrets Manager (or Parameter Store) at boot using the instance's IAM role.
4. Health check path matters â€” if the ALB health check hits a path that requires DB connectivity and RDS isn't ready yet at boot, instances can get marked unhealthy and cycle repeatedly; consider a lightweight /health endpoint that doesn't depend on the DB, or add sufficient ASG/ALB health check grace period.
5. NAT Gateway cost â€” a NAT Gateway per AZ is best practice for HA but adds cost; a single shared NAT Gateway is common for lab/dev environments as a cost trade-off (call this out explicitly if you did it).
6. Multi-AZ RDS failover isn't instant â€” expect ~60â€“120 seconds of downtime during failover; the app should have basic retry logic for DB connections rather than assuming an always-instant connection. 
7. ASG scale-in can kill connections mid-request â€” enable connection draining / deregistration delay on the target group so in-flight requests finish before an instance is removed.
